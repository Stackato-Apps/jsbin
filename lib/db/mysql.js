...
module.exports = utils.inherit(Object, {
  ...
  connect: function (fn) {
    ...
    // Ensure the MySQL database structure is setup in a Stackato-env
    // -- should be moved to it's own script in a pre-running hook.
    if(process.env.VCAP_APP_PORT){
      fs.readFile('/app/app/build/full-db-v3.mysql.sql', function(err, queryData){
        self.connection.query(queryData.toString(), function(err, result){
          console.log(result);
        });
      });
    }
